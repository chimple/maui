...
class PlayedGamesScoreDisplayState extends State<PlayedGamesScoreDisplay> {
  ...
  void _initData() async {
    ...
    Map<String, List<Score>> fetchData =
        await ScoreRepo().getScoreMapByUser(loggedInUser.id);
  ...
  Widget build(BuildContext context) {
    ...
    _scores.forEach((k, v) {
      ...
      scoreWidgets.add(new Container(
        ...
        child: ControlledExpansionTile(
            ...
            children: v.map((f) {
              return scoreHistory(f.myUser, f.otherUser, f.myScore,
                  f.otherScore, f.game, f.playedAt);
            }).toList(growable: false)),
      ));
    });
    return new ListView(children: scoreWidgets);
  }

  Widget scoreHistory(String myUser, String otherUser, int myScore,
      int otherScore, String game, int playedAt) {
        ...
        child: new Row(
          ...
          children: <Widget>[
            ...
            Expanded(
              child: new Container(
                margin:
                    const EdgeInsets.only(top: 4.0, bottom: 4.0, left: 25.0),
                child: new Text(
                  '${myScore}',
                  style: new TextStyle(
                      fontSize: 16.0,
          ...
          children: <Widget>[
            ...
            new Container(
              ...
              child: new Container(
                ...
                child: new CircleAvatar(
                  backgroundColor: Colors.white,
                  backgroundImage: otherUserImagePath == ''
                      ? null
                      : new FileImage(new File(
                          '/data/user/0/sutara.org.maui/app_flutter/${otherUser}.png')),
                ),
              ),
            ),
          ],
